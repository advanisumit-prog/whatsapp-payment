...
async function readPaymentsBatch(imagePaths) {
    ...
    // Both attempts failed — a quota limit, a network drop, a bad key.
    //
    // This is NOT the same as "not a payment", and must not be recorded as one:
    // doing so deletes the image and marks it seen, so a run that hits a quota
    // limit silently discards every payment it had already downloaded.
    //
    // The error flag tells the caller to leave these images alone so the next
    // run picks them up.
    console.log(`  ❌ Giving up on this batch of ${imagePaths.length} image(s) — NOT treating them as non-payments; they will be retried next run.`);

    return imagePaths.map(() => ({ isPayment: false, error: true }));
}
