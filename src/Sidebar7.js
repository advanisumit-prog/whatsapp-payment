 ...
 *
 * Walking outward from each row kept missing dividers, because it depends on
 * where the divider sits relative to that row. Reading top to bottom and
 * carrying the current date forward is how a person reads the chat, so a
 * divider anywhere above a message applies to it.
 */
async function buildDateMap(container) {
            ...
            for (const child of node.children) {
                ...
                if (id) {
                    if (!seen.has(id)) {
                        ...
                        out.push({ type: "msg", id, time: meta ? (meta.innerText || "").trim() : null });
                    }
                    continue;
                }
    ...
    for (const item of items) {
        ...
        if (current) map.set(item.id, { date: current, timeText: item.time || null });
    }
