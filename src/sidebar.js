...
class Sidebar {
    ...
    async collectAndDownloadNewImages(page, database, downloader, groupName, staticPartyLabel = null, isFirstChat = false) {
        ...
        let todayHits = 0;
        // Genuine safety ceiling only — the real stop conditions are
        // reachedToday (date-based) and the scroll-position bottom check
        // further down. hitMaxScrolls tracks whether we actually got a
        // real stop signal or just ran out of budget, so that running out
        // is loud instead of silently truncating collection.
        const maxScrollsDown = 600;
        let hitMaxScrolls = true;
        ...
        for (let i = 0; i < maxScrollsDown; i++) {
            ...
            // NOTE: there used to be a backstop here that stopped collection
            // after 25 scroll passes with no NEW image found. That heuristic
            // only counted images, not scroll position or date, so a long
            // stretch of ordinary text messages (very normal in a busy chat)
            // looked identical to "we've reached the end" and cut collection
            // off early — silently losing every image below that point, even
            // ones still well within the date window. The scroll-position
            // bottom check just below is the correct way to detect the real
            // end of the chat, so that's the only thing this loop trusts now.
        ...
        if (hitMaxScrolls) {
            console.log(`  ⚠️ Hit the ${maxScrollsDown}-scroll cap without confirming we reached the bottom or the window edge — some messages below this point were likely never scanned. Re-run to pick them up, or raise maxScrollsDown if this keeps happening.`);
        }
