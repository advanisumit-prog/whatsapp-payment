...
async function clickLoadOlderMessages(page) {
    ...
    // The visible text sits inside a button/clickable wrapper. Clicking the text
    // node itself often does nothing, so find the real control and dispatch the
    // click in-page.
    const label = await page.evaluate(() => {

        const wanted = /get older messages from your phone|click here to get older messages/i;

        for (const node of document.querySelectorAll("div, span, button, a")) {

            const text = (node.innerText || "").trim();
            if (!text || text.length > 80 || !wanted.test(text)) continue;
            if (node.children.length > 2) continue;      // innermost element only
    ...
    console.log(`  📥 Clicked "${label}" — fetching history from the phone...`);
    ...
    async collectAndDownloadNewImages(page, database, downloader, groupName, staticPartyLabel = null) {
        ...
        for (let i = 0; i < maxScrollsUp && !reachedCutoff; i++) {
            ...
            if (scrollTop !== null && scrollTop <= 5) {
                ...
                const clicked = await clickLoadOlderMessages(page);

                if (clicked) {
                    const grew = await waitForMoreMessages(container, page, before, 60000);
                    ...
                    console.log("  ⏳ Clicked load-older but no new messages arrived.");
                }
