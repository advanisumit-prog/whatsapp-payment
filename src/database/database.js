...
class Database {
    ...
    initialize() {
        ...
        return new Promise((resolve, reject) => {
            ...
            this.db.run(createSql, err => {
                ...
                // Existing databases created seen_media without image_hash.
                this.db.run("ALTER TABLE seen_media ADD COLUMN image_hash TEXT", () => {});
                this.db.run("CREATE INDEX IF NOT EXISTS idx_seen_hash ON seen_media (image_hash)", () => {});
    ...
    /** Generic read helper, used by the end-of-run report. */
    query(sql, params = []) {
    ...
    /**
     * Newest message date already recorded for a group, as "YYYY-MM-DD".
     * Used to advance the scrape window automatically between runs.
     */
    getLastMessageDate(groupName) {

        return new Promise((resolve, reject) => {

            const sql = `
                SELECT MAX(DATE(message_timestamp)) AS last_date
                FROM payments
                WHERE group_name = ? AND message_timestamp IS NOT NULL
            `;

            this.db.get(sql, [groupName], (err, row) => {
                if (err) return reject(err);
                resolve(row && row.last_date ? row.last_date : null);
            });
    ...
    /**
     * Is this UTR already recorded for THIS group? A payment forwarded twice
     * inside one group is the same payment, not a second one — recording it
     * again double-counts the day's total.
     */
    utrExistsInGroup(utr, groupName) {

        return new Promise((resolve, reject) => {

            if (!utr || !groupName) return resolve(false);

            const sql = `
                SELECT 1 FROM payments
                WHERE utr = ? AND group_name = ?
                LIMIT 1
            `;

            this.db.get(sql, [utr, groupName], (err, row) => {
                if (err) return reject(err);
                resolve(!!row);
            });
    ...
    /** Full payment row for an identical image already processed elsewhere. */
    findPaymentByHash(hash) {
        ...
        return new Promise((resolve, reject) => {
            ...
            const sql = `
                ...
                WHERE image_hash = ? AND is_payment = 1
                LIMIT 1
            `;
    ...
    /** True if an identical image was already judged NOT to be a payment. */
    seenNonPaymentByHash(hash) {

        return new Promise((resolve, reject) => {

            if (!hash) return resolve(false);

            const sql = `SELECT 1 FROM seen_media WHERE image_hash = ? LIMIT 1`;
    ...
    markSeen(messageId, groupName, imageName, imageHash = null, reason = "not_a_payment") {
        ...
        return new Promise((resolve, reject) => {
            ...
            const sql = `
                ...
                (message_id, group_name, image_name, image_hash, reason, seen_at)
                VALUES (?, ?, ?, ?, ?, ?)
            `;

            this.db.run(sql, [messageId, groupName, imageName, imageHash, reason, stamp], function (err) {
                if (err) return reject(err);
                resolve(this.changes);
            });
