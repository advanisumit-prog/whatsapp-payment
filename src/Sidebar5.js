...
async function readNearestDividerDate(row) {
    ...
    // Done in-page rather than via an XPath preceding:: axis, which does not
    // resolve reliably through Playwright's chained-locator scoping. Walks back
    // through previous siblings, then up a level and back again, until it finds
    // a date divider: a container with a span[dir="auto"] but no message inside.
    const dividerText = await row.evaluate(el => {

        const isDivider = node => {
            if (!node || node.nodeType !== 1) return false;
            if (node.getAttribute("role")) return false;
            if (node.querySelector('[data-testid="msg-container"], [data-testid="image-album"]')) return false;
            return !!node.querySelector('span[dir="auto"]');
        };
        ...
        while (node && hops < 5) {
            ...
            while (prev) {
                if (isDivider(prev)) {
                    const span = prev.querySelector('span[dir="auto"]');
    ...
    return dividerText ? parseDividerDate(dividerText) : null;
}
...
class Sidebar {
    ...
    async collectAndDownloadNewImages(page, database, downloader, groupName, staticPartyLabel = null) {
        ...
        for (let i = 0; i < 15; i++) {
            const count = await container.locator('[data-testid="msg-container"], [data-testid="image-album"]').count();
            if (count === previousBottomCount) {
                stableBottomCount++;
        ...
        for (let i = 0; i < maxScrollsDown; i++) {
            ...
            for (let r = 0; r < itemCount; r++) {
                ...
                if (role !== 'row') {
                    ...
                    if (dividerText) {
                        ...
                        if (parsedDate) {
                            ...
                            if (parsedDate >= endDateExclusive) {
                ...
                if (albumCount > 0) {
                    ...
                    const mediaCount = await albumRow.locator('[data-testid="media-url-provider"]').count();
                ...
                const mediaCount = (await allMedia.count()) - (await quotedMedia.count());
            ...
            console.log(`  Scroll down ${i + 1}: ${itemCount} items rendered, ${imageMap.size} unique images captured so far`);

            const bottomCheckCount = await container.locator('[data-testid="msg-container"], [data-testid="image-album"]').count();
            if (bottomCheckCount === previousBottomCount2) {
                stableBottomCount2++;
                if (stableBottomCount2 >= 4) {
