...
class Sidebar {
    ...
    async collectAndDownloadNewImages(page, database, downloader, groupName, staticPartyLabel = null) {
        ...
        for (let i = 0; i < maxScrollsUp && !reachedCutoff; i++) {
            ...
            // The link can appear part-way up, not only at scrollTop 0, so look
            // for it on every pass rather than waiting to hit the very top.
            ...
            if (await clickLoadOlderMessages(page)) {
                ...
                console.log("  ⏳ Clicked load-older but no new messages arrived yet.");
            }
