...
class Downloader {
    ...
    async download(groupName, image, index, previousSrc = null, messageId = null, mediaIndex = 0) {
        ...
        const providers = await image.locator.locator('[data-testid="media-url-provider"]').all();
        const thumbs = await image.locator.locator('[data-testid="image-thumb"]').all();

        // An image WhatsApp has not fetched yet has an image-thumb but no
        // media-url-provider. In an album only some photos are preloaded, so
        // whichever list is LONGER is the one that covers every photo — using
        // providers alone silently drops the unloaded ones.
        let allMediaLocators = providers;

        if (thumbs.length > providers.length) {
            allMediaLocators = thumbs;
            console.log(`  ⏳ Using image-thumb (${thumbs.length} vs ${providers.length} loaded) to reach every photo.`);
        }
        ...
        // An album of 5 photos renders only 4 tiles — the last carries a "+1"
        // overlay. So the extra photos have no thumbnail to click: open the last
        // tile and page forward in the viewer to reach them.
        ...
        for (let step = 0; step < advanceBy; step++) {
            ...
            // The next image may still be downloading, so wait for src to change.
